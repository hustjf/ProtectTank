...
bool SecondScene::init()
{
    ...
    this->addHome();
    this->scheduleUpdate();
    this->schedule(schedule_selector(SecondScene::gameLogic), 1.5f);
    this->currentLevel = 0;
...
void SecondScene::gameLogic(float dt)
{
...
void SecondScene::FollowPath(Node *sender)
{
    DataModel *m = DataModel::getModel();
    ...
    if (waypoint!=nullptr) {
        ...
        if (m->home->homeHP<=0) {
            ...
            restartButton->addTouchEventListener(this, toucheventselector(SecondScene::restartEvent));
            mainMenuButton->addTouchEventListener(this, toucheventselector(SecondScene::mainMenuEvent));
            this->cleanup();
        }else{
...
void SecondScene::update(float dt)
{
    ...
    if (this->currentLevel==-1&&m->targets.size()==0) {
        ...
        restartButton->addTouchEventListener(this, toucheventselector(SecondScene::restartEvent));
        continueButton->addTouchEventListener(this, toucheventselector(SecondScene::continueEvent));
...
void SecondScene::restartEvent(Widget* target, TouchEventType type)
{
...
void SecondScene::continueEvent(Widget* target, TouchEventType type)
{
    if(type==TouchEventType::TOUCH_EVENT_ENDED){
        DataModel *m = DataModel::getModel();
        m->clear();
